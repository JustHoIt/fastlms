...
public class AdminTakeCourseController extends BaseController {
    ...
    public String list(Model model, TakeCourseParam parameter, BindingResult bindingResult) {
        ...
        List<TakeCourseDto> takeCourseDtoList = takeCourseService.list(parameter);


        long totalCount = 0;

        if (!CollectionUtils.isEmpty(takeCourseDtoList)) {
            totalCount = takeCourseDtoList.get(0).getTotalCount();
        }
        String queryString = parameter.getQueryString();
        String pagerHtml = getPagerHtml(totalCount, parameter.getPageSize(), parameter.getPageIndex(), queryString);

        PageUtil pageUtil = new PageUtil(totalCount, parameter.getPageSize(), parameter.getPageIndex(), queryString);
        model.addAttribute("list", takeCourseDtoList);
